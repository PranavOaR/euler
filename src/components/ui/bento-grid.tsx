...
const BentoCard = ({
  ...
  >
    <div className={className?.includes('dashboard-card') ? "absolute inset-0 overflow-hidden rounded-xl" : ""}>{background}</div>
    <div className={cn(
      "pointer-events-none z-10 flex transform-gpu flex-col gap-1 transition-all duration-300 group-hover:-translate-y-10",
      className?.includes('dashboard-card') ? "p-6 pt-8" : "p-6"
    )}>
