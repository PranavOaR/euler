...
import { signInWithGoogle, verifyEmailCode } from "@/hooks/useAuth";
...
export const SignInPage = ({ className }: SignInPageProps) => {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (email) {
      // For demo purposes, we'll go directly to code verification
    ...
    if (value.length <= 1) {
      ...
      if (index === 5 && value) {
        ...
        if (isComplete) {
          ...
          try {
            const { success, error } = await verifyEmailCode(email, codeString);
            if (success) {
                ...
                {step === "email" ? (
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-xl md:text-2xl text-white/90 leading-relaxed drop-shadow-lg">Building the Future of Automated Capital</p>
                    </div>
                  ...
                  >
                    <div className="space-y-1">
                      <h1 className="text-6xl md:text-8xl font-bold text-white mb-6 tracking-tight drop-shadow-2xl">We sent you a code</h1>
                      <p className="text-xl md:text-2xl text-white/90 leading-relaxed drop-shadow-lg">Please enter it below</p>
                    </div>
